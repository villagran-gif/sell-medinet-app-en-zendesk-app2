(function () {
  ...
  const FIELD_KEYS = {
    ...
    // Antropométricos (preferir Deal; fallback a Contact). Excluir nombres con '#'.
    peso: [FIELD_IDS.peso, FIELD_IDS.pesoLegacyNum, FIELD_IDS.pesoLegacyStr, "Peso", "PESO", "Peso (kg)", "PESO (KG)"],
    talla: [FIELD_IDS.talla, FIELD_IDS.tallaLegacyNum, FIELD_IDS.tallaLegacyStr, "Talla", "Estatura", "Altura", "ESTATURA", "ALTURA"],
  };
  ...
  // ============================================================
  // Antropométricos: resolver Peso/Talla desde Deal/Contact
  // Regla:
  //  - Tomar el PRIMER campo con valor NO vacío cuyo nombre contenga:
  //      Peso  -> /peso/i
  //      Talla -> /(talla|estatura|altura)/i
  //  - EXCLUIR campos cuyo nombre contenga '#'
  //  - Preferir: Deal -> Contact
  //  - También probamos IDs conocidos (por si viene por id y no por nombre)
  // ============================================================

  function getCandidateMaps(entity) {
    return [
      entity && entity.custom_fields,
      entity && entity.customFields,
      entity && entity.custom_field_values,
      entity && entity.customFieldValues,
    ].filter((m) => m && isObject(m));
  }

  function findFirstByNameRegex(entity, regex) {
    if (!entity) return "";
    const maps = getCandidateMaps(entity);
    ...
    for (const map of maps) {
      for (const [k, v] of Object.entries(map)) {
        ...
        // ✅ excluir deprecados con '#'
        if (key.includes("#")) continue;

        if (regex.test(key)) {
          const valStr = toStringValue(v);
          if (valStr && valStr.trim() !== "") return v;
        }
      }
    }
    return "";
  ...
  function resolvePeso(deal, contact) {
    // 1) IDs / aliases conocidos
    const dealKnown = toStringValue(getField(deal, FIELD_KEYS.peso));
    if (dealKnown) return dealKnown;

    // 2) Buscar por nombre (sin '#')
    const dealByName = toStringValue(findFirstByNameRegex(deal, /peso/i));
    if (dealByName) return dealByName;

    // 3) Contact fallback
    const contactKnown = toStringValue(getField(contact, FIELD_KEYS.peso));
    if (contactKnown) return contactKnown;

    const contactByName = toStringValue(findFirstByNameRegex(contact, /peso/i));
    return contactByName || "";
  }

  function resolveTalla(deal, contact) {
    const rx = /(talla|estatura|altura)/i;

    const dealKnown = toStringValue(getField(deal, FIELD_KEYS.talla));
    if (dealKnown) return dealKnown;

    const dealByName = toStringValue(findFirstByNameRegex(deal, rx));
    if (dealByName) return dealByName;

    const contactKnown = toStringValue(getField(contact, FIELD_KEYS.talla));
    if (contactKnown) return contactKnown;

    const contactByName = toStringValue(findFirstByNameRegex(contact, rx));
    return contactByName || "";
  }
  ...
  async function onUploadClick() {
    ...
    try {
      ...
      const peso = resolvePeso(deal, contact);
      const talla = resolveTalla(deal, contact);
      ...
      const payload = {
        ...
        peso,
        talla,
        deal_url: `https://clinyco.zendesk.com/sales/deals/${Number(deal.id)}`,
        contact_url: `https://clinyco.zendesk.com/sales/contacts/${Number(contact.id)}`,
